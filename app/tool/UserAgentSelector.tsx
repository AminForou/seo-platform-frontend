...
const UserAgentSelector: React.FC<UserAgentSelectorProps> = ({ userAgent, setUserAgent }) => {
  ...
  const [selectedOption, setSelectedOption] = useState<UserAgentOption>('default');
  ...
  const handleCustomUserAgentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="space-y-2 mt-4 mb-6">
      <label htmlFor="userAgent" className="block text-sm font-medium text-gray-700 flex items-center">
        Select User-Agent
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          <span className="block truncate">{predefinedUserAgents.find(ua => ua.value === selectedOption)?.label}</span>
          ...
          <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
            {predefinedUserAgents.map((ua) => (
              <div
                key={ua.value}
                className={`${
                  selectedOption === ua.value ? 'text-white bg-indigo-600' : 'text-gray-900'
                } cursor-default select-none relative py-2 pl-3 pr-9 hover:bg-indigo-100`}
                onClick={() => handleUserAgentChange(ua.value as UserAgentOption)}
              >
                <span className={`block truncate ${selectedOption === ua.value ? 'font-semibold' : 'font-normal'}`}>
                  {ua.label}
                </span>
              </div>
            ))}
      ...
      {selectedOption === 'custom' && (
        <>
          <label htmlFor="customUserAgent" className="block text-sm font-medium text-gray-700 mt-2">
