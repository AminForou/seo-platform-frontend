...
// Based on "Improving the Rainbow" by Alan Zucconi and "More accurate Iridescence" by Julia Poo
...
const RainbowComponent = forwardRef<THREE.Mesh, RainbowProps>(({ 
  startRadius = 0, 
  endRadius = 0.5, 
  emissiveIntensity = 2.5, 
  fade = 0.25, 
  ...
  const length = Math.hypot(width, height) + 1.5 // add 1.5 due to motion of the rainbow
  
  useFrame((state, delta) => (material.current && ((material.current as any).time += delta * (material.current as any).speed)))
